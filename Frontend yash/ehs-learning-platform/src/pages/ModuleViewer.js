...
const ModuleViewer = () => {
  ...
  useEffect(() => {
    const fetchModuleData = async () => {
      try {
        ...
        // Check access
        if (moduleResponse.data.hasAccess === false) {
          setError('You do not have access to this module. Please contact your administrator.');
          setLoading(false);
          return;
        }
        ...
        const totalComponents = moduleData.components?.length || 0;
        const progressPercentage = totalComponents > 0 
          ? (completedComponents.length / totalComponents) * 100 
          : 0;
      ...
      try {
        ...
        if (component.type === 'PRE_ASSESSMENT' || component.type === 'POST_ASSESSMENT') {
          ...
          try {
            ...
            setActiveComponent(materialData);
          } catch (err) {
            console.error('Error loading learning materials:', err);
            setComponentError('Failed to load learning materials.');
          }
        } else {
  ...
  const handleSubmitAssessment = async () => {
    ...
    try {
      ...
      const formattedAnswers = Object.entries(assessmentAnswers).map(([questionId, userAnswer]) => ({
        questionId,
        userAnswer
      }));
      ...
      const response = await assessmentService.submitAnswers(activeComponent.id, submission);
      const result = response.data;
      
      setAssessmentComplete(true);
      setAssessmentScore(result.overallScore);
      setAssessmentFeedback(result.questionResults);
      ...
      setProgress(prev => {
        const newCompleted = Math.ceil((prev / 100) * moduleData.components.length) + 1;
        return (newCompleted / moduleData.components.length) * 100;
      });
      
      // If this was the last component, show completion message
      if (currentStep === moduleData.components.length - 1) {
        // Handle module completion - could show a modal or redirect
      }
    } catch (err) {
        ...
        {componentLoading ? (
          ...
          <>
            <Box mb={3}>
              ...
              {activeComponent.estimatedDuration && (
                <Box sx={{ display: 'flex', alignItems: 'center', mt: 1 }}>
                  <ScheduleIcon fontSize="small" color="action" />
                  <Typography variant="body2" color="text.secondary" sx={{ ml: 1 }}>
                    Estimated duration: {activeComponent.estimatedDuration} minutes
                  </Typography>
                </Box>
              )}
            ...
            {(activeComponent.type === 'PRE_ASSESSMENT' || activeComponent.type === 'POST_ASSESSMENT') && (
              assessmentComplete ? (
                <Box>
                  ...
                  {assessmentFeedback && assessmentFeedback.length > 0 && (
                    <Box mt={3}>
                        ...
                        <Card key={index} variant="outlined" sx={{ mb: 2 }}>
                          <CardContent>
                            <Typography variant="body1" gutterBottom>
                              <strong>Question {index + 1}:</strong> {feedback.text}
                            </Typography>
                            <Typography variant="body2" color={feedback.isCorrect ? "success.main" : "error.main"}>
                ...
                <Box>
                  {activeComponent.questions && activeComponent.questions.length > 0 ? (
                    activeComponent.questions.map((question, qIndex) => (
                  ...
                  <Box mt={3} display="flex" justifyContent="space-between">
                    ...
                    <Button
                      ...
                      disabled={componentLoading || !activeComponent.questions || activeComponent.questions.length === 0}
                    >
